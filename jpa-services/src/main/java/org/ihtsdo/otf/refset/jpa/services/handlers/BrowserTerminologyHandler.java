...
public class BrowserTerminologyHandler implements TerminologyHandler {
  ...
  public ConceptList resolveExpression(String expr, String terminology,
    ...
    WebTarget target =
        client.target(url + "/expressions/en-edition/" + version
            + "/execute/brief");
    /*
     * client.target(url + "/" + branch + "/" + version + "/concepts?escg=" +
  ...
  public Concept getFullConcept(String terminologyId, String terminology,
    ...
    final WebTarget target =
        client.target(url + "/en-edition/" + version + "/concepts/"
            + terminologyId);
    final Response response =
    ...
    if (doc.get("relationships") != null) {
      for (final JsonNode relNode : doc.get("relationships")) {
        ...
        rel.setCharacteristicTypeId(relNode.get("charType").get("conceptId")
            .asText());
        // Only keep INFERRED_RELATIONSHIP rels TODO: Only keep
        // INFERRED_RELATIONSHIP rels - change this; more than two types
        ...
        destination.setName(relNode.get("target").get("defaultTerm").asText()); // TODO
                                                                                // =
                                                                                // contains
                                                                                // semantic
                                                                                // type
        destination.setDefinitionStatusId(relNode.get("target")
            .get("definitionStatus").asText()); // TODO - this should be an id,
                                                // not "Primitive"
  ...
  public ConceptList findConceptsForQuery(String query, String terminology,
    ...
    // TODO useTerm
    /*
     * boolean useTerm = true; String localQuery = query; if
     * (localQuery.matches("\\d+[01]0\\d")) { useTerm = false; } else if
     * (localQuery.matches("\\d+[01]0\\d\\*")) { // remove * if this is an id
     * search (only digits) localQuery = localQuery.replace("*", ""); useTerm =
     * false; }
     */
    ...
    final WebTarget target =
        client.target(url + "/en-edition/" + version + "/descriptions?query="
            + query
            + "&searchMode=partialMatching&lang=english&statusFilter=english&"
     ...
     * "/concepts?term=" + URLEncoder.encode(localQuery,
     * "UTF-8").replaceAll(" ", "%20") + "&offset=" + localPfs.getStartIndex() +
     * "&limit=" + localPfs.getMaxResults() + "&expand=pt()")
     * 
     * :
     * 
     * client.target(url + "/" + branch + "/" + version + "/concepts?escg=" +
    ...
    for (final JsonNode conceptNode : doc.get("matches")) {
       ...
       * ( "effectiveTime").asText()));
       */// TODO: no effectiveTime nullable=true
